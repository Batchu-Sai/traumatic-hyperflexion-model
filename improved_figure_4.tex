...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[scale=1.4, every node/.style={font=\sffamily, font=\fontsize{8}{8}\selectfont}]
        ...
        % Moment arm r_c (dashed line) - moved to avoid T_n label
        \coordinate (cordPoint) at ($(TnTR)-(0.3,0.1)$);
        \draw[dashed, line width=0.5pt] (ICR) -- (cordPoint);
        \coordinate (rcMid) at ($(ICR)!0.6!(cordPoint)$);
        \node[right=2pt] at (rcMid) {$r_c$};
        
        % Spinal cord force vector (CORRECTED: pointing posteriorly)
        \coordinate (cordForceMid) at ($(cordPoint)!0.5!($(Tnp1TR)-(0.3,0.1)$)$);
