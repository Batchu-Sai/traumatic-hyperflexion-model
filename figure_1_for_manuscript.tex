% Figure 1: Model Schematic - Copy this code block into your manuscript
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[scale=1.4, every node/.style={font=\sffamily, font=\fontsize{8}{8}\selectfont}]
        ...
        % Top vertebra Tn+1 - INCREASED SPACING
        \coordinate (Tnp1BL) at ($(ICR)+(0,\h+0.6)$);
        ...
        % Disc-ligament complex - ELONGATED in Y-direction with MORE SPACING
        \coordinate (discStart) at (TnTL);
        \coordinate (discEnd) at (Tnp1BL);
        \coordinate (discMid) at ($(discStart)!0.5!(discEnd)$);
        
        % Spring-damper representation - ELONGATED in Y-direction with MORE SPACING
        \draw[decorate, decoration={coil,aspect=0.15,segment length=6pt,amplitude=8pt}] 
        (discStart) -- ($(discMid)-(0.8,0)$);
        \draw[line width=1.5pt] ($(discMid)-(0.8,0)$) --++ (0.3,0);
        \draw[line width=1.5pt] ($(discMid)-(0.7,0.25)$) rectangle ++(1.4,0.5);
        \draw[line width=1.5pt] ($(discMid)+(0.7,0)$) -- (discEnd);
        
        % External moment M_ext (curved arrow) - moved further out for MORE SPACING
        \draw[->, >=Stealth, line width=1.5pt, color=blue!60!black] 
            ($(Tnp1TL)+(2.5,0.1)$) arc[start angle=110, end angle=30, radius=1.3];
        \node at ($(Tnp1TL)+(3.5,0.3)$) {$M_{\text{ext}}$};
        
        % Disc-ligament moment M_disc-lig (curved arrow) - moved further out for MORE SPACING
        \draw[->, >=Stealth, line width=1.2pt, color=green!60!black] 
            ($(discMid)+(1.0,0.8)$) arc[start angle=45, end angle=135, radius=1.5];
        \node at ($(discMid)+(-1.0,0.6)$) {$M_{\text{disc-lig}}$};
